...
public partial class ps_rfq
{
    ...
    {
        StringBuilder strSql = new StringBuilder();
        strSql.Append("Update ps_RFQVend set VendorRemark = @VendorRemark,VendorUnitPrice = @VendorUnitPrice ");
        strSql.Append(" where ID=@ID");
        SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.Int,4),
    ...
    public bool ComfirmVendorResponseStatus(int ID)
    {
        StringBuilder strSql = new StringBuilder();
        strSql.Append("UPDATE B SET Response='R',RespondDate = GETDATE() from ps_RFQItem  A inner join ps_RFQVend  B on A.RFQNum = B.RFQNum AND A.RFQLine = B.RFQLine");
    ...
    public DataSet GetListByRFQVendorID(int id, out int recordCount)
    {
        StringBuilder strSql = new StringBuilder();
        strSql.Append("select * FROM  v_RFQ");
        if (id > 0)
        {
            strSql.Append(" where RFQVendorID=" + id);
        }
        recordCount = Convert.ToInt32(DbHelperSQL.GetSingle(PagingHelper.CreateCountingSql(strSql.ToString())));
        return DbHelperSQL.Query(PagingHelper.CreatePagingSql(recordCount, 1, 1, strSql.ToString(), " id"));
    }
