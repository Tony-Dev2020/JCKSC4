...
public partial class select_order_list : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["DepotCatID"] != null && Session["DepotCatID"].ToString() != "")
        {
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        this.Literal_pa.Text = MyConvert(bll.GetTitleSum(_strWhere + " and  status<>10", "sum(payable_amount)"));
        this.Literal_ra.Text = MyConvert(bll.GetTitleSum(_strWhere + " and  status<>10", "sum(real_amount)"));
        this.Literal_oa.Text = MyConvert(bll.GetTitleSum(_strWhere + " and  status<>10", "sum(order_amount)"));
        //绑定页码
    ...
    protected string CombSqlTxt(int _depot_category_id, int _depot_id,  int _status,  string _note_no, string _start_time, string _stop_time)
    {
        ...
        if (!string.IsNullOrEmpty(_note_no))
        {
            ...
            strTemp.Append(" and (order_no like  '%" + _note_no + "%' or epicor_order_no like  '%" + _note_no + "%' or  remark like  '%" + _note_no + "%'  or address like  '%" + _note_no + "%'  or message like  '%" + _note_no + "%' or id in (select  order_id from v_OrderGoods where commercialStyle + commercialcolor+goods_title+product_name  like'%" + _note_no + "%'  ) )");
        }
