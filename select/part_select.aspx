<%@ Page Language="C#" AutoEventWireup="true" CodeFile="part_select.aspx.cs" Inherits="select_part_select" %>
...
<script type="text/javascript">
    ...
    function selPart(partno, partname) {
        window.parent.$("#txtPartNo").val(partno);
        window.parent.$("#txtPartDesc").val(partname);
...
<body>
   ...
   <dl class="seachform"> 
    ...
    <asp:DropDownList ID="ddldepot_category_id"  runat="server" Enabled="false" AutoPostBack="True" onselectedindexchanged="ddldepot_category_id_SelectedIndexChanged">
    </asp:DropDownList>
    </span>
    </dd> 
     <dd><label>产品种类</label>  
   ...
   <dd class="cx"><asp:Button ID="lbtnSearch" runat="server" CssClass="scbtn" onclick="btnSearch_Click" Text="查询"></asp:Button>   
        ...
        <tr>
            ...
            <td><input id="btnSelCus" onclick="selPart('<%# Eval("product_no")%>','<%# Eval("product_name")%>')" type="button" class="btn green" value="选择" /> </td>
                <%--<asp:LinkButton ID="lbtnAcceptCa" runat="server" CommandArgument='<%# Eval("id")%>' OnClientClick="return confirm('是否真的要设置为缺货？设置缺货后客户讲无法购买！')" onclick="lbtnAcceptCa_Click"><%# Eval("status").ToString().Trim() == "0" ? "<font color =red>[设置缺货]</font>" : ""%></asp:LinkButton>
                <asp:LinkButton ID="lbtnRefuseCa" runat="server" CommandArgument='<%# Eval("id")%>' OnClientClick="return confirm('是否真的要设置为有货？设置后客户可购买！')" onclick="lbtnRefuseCa_Click"><%# Eval("status").ToString().Trim() == "1" ? "<font color =#4B6A11>[设置有货]</font>" : ""%></asp:LinkButton>
                <asp:LinkButton ID="lbtnAcceptCaDG" runat="server" CommandArgument='<%# Eval("id")%>' OnClientClick="return confirm('是否真的要设置为暂停订购？设置暂停订购后客户无法看见该产品！')" onclick="lbtnAcceptCaDG_Click"><%# Eval("is_xs").ToString().Trim() == "0" ? "<font color =red>[设置暂停]</font>" : ""%></asp:LinkButton>
                <asp:LinkButton ID="lbtnRefuseCaDG" runat="server" CommandArgument='<%# Eval("id")%>' OnClientClick="return confirm('是否真的要设置为可以订购？设置后客户可以看见该产品！')" onclick="lbtnRefuseCaDG_Click"><%# Eval("is_xs").ToString().Trim() == "1" ? "<font color =#4B6A11>[设置可订]</font>" : ""%></asp:LinkButton>--%>
            </td>
        </tr>      
	   </ItemTemplate>
<FooterTemplate>
