...
public partial class select_vendor_select : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            TreeBind(this.CatID); //绑定地区

            this.ddlCategoryId.SelectedValue = this.CatID.ToString();
            RptBind("CatID=" + this.CatID + CombSqlTxt(this.status, this.CatID, this.keywords), "CatID asc,id desc");
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        string pageUrl = Utils.CombUrlTxt("vendor_select.aspx", "status={0}&CatID={1}&keywords={2}&page={3}", this.status.ToString(), this.CatID.ToString(), this.keywords, "__id__");
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("vendor_select.aspx", "status={0}&CatID={1}&keywords={2}", this.status.ToString(), this.CatID.ToString(), txtKeywords.Text));
    }

    //筛选地区
    protected void ddlCategoryId_SelectedIndexChanged(object sender, EventArgs e)
    {
        Response.Redirect(Utils.CombUrlTxt("vendor_select.aspx", "status={0}&CatID={1}&keywords={2}",
    ...
    protected void txtPageNum_TextChanged(object sender, EventArgs e)
    {
        ...
        Response.Redirect(Utils.CombUrlTxt("vendor_select.aspx", "status={0}&CatID={1}&keywords={2}", this.status.ToString(), this.CatID.ToString(), this.keywords));
    }
    ...
    protected void lbtnDelCa_Click(object sender, EventArgs e)
    {
        ...
        if (!bllpp.ExistsMD())//查找是否存在用户
        {
            ...
            mym.JscriptMsg(this.Page, " 成功删除供应商：" + name + "", Utils.CombUrlTxt("vendor_select.aspx", "status={0}&CatID={1}&keywords={2}&page={3}", this.status.ToString(), this.CatID.ToString(), this.keywords, this.page.ToString()), "Success");
        }
        else
        {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        mym.JscriptMsg(this.Page, " 排序保存成功", Utils.CombUrlTxt("vendor_select.aspx", "status={0}&CatID={1}&keywords={2}&page={3}", this.status.ToString(), this.CatID.ToString(), this.keywords, this.page.ToString()), "Success");
    }
