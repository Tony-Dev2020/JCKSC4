...
public partial class depotmanager_back_go : System.Web.UI.Page
{
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        string pageUrl = Utils.CombUrlTxt("back_go.aspx", "product_category_id={0}&note_no={1}&page={2}", this.product_category_id.ToString(), this.note_no, "__id__");
    ...
    {
        int _pagesize;
        if (int.TryParse(Utils.GetCookie("back_go_page_size"), out _pagesize))
        {
            if (_pagesize > 0)
            {
                return _pagesize;
    ...
    {
        //Response.Redirect(Utils.CombUrlTxt("back_go.aspx", "product_category_id={0}&note_no={1}", this.product_category_id.ToString(), txtNote_no.Text));
        Response.Redirect(Utils.CombUrlTxt("back_go.aspx", "product_category_id={0}&note_no={1}&company={2}", this.ddlproduct_category_id.SelectedValue, txtNote_no.Text, this.ddldepot_category_id.SelectedValue));
    }
    ...
    protected void txtPageNum_TextChanged(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
        {
            if (_pagesize > 0)
            {
                Utils.WriteCookie("back_go_page_size", _pagesize.ToString(), 14400);
            }
        }
        Response.Redirect(Utils.CombUrlTxt("back_go.aspx", "product_category_id={0}&note_no={1}", this.product_category_id.ToString(), txtNote_no.Text));
    }
