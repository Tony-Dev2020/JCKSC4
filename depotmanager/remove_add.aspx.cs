...
public partial class depotmanager_remove_add : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (Convert.ToInt32(Session["DepotID"]) == 0 && Convert.ToInt32(Session["DepotCatID"]) == 0)//公司用户
            {
                ...
                RptBind("id>0" + CombSqlTxt(this.product_category_id, this.note_no,this.company), "id desc");
            }
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        //EpicorRequest.PartData[] partDatas =  bllEpicor.GetPartNum("", "");
        ...
        string pageUrl = Utils.CombUrlTxt("remove_add.aspx", "product_category_id={0}&note_no={1}}&company={2}&page={3}", this.product_category_id.ToString(), this.note_no, this.company, "__id__");
        PageContent.InnerHtml = Utils.OutPageList(this.pageSize, this.page, this.totalCount, pageUrl, 8);
    }
    #endregion

    #region 组合SQL查询语句==========================
    protected string CombSqlTxt(int _product_category_id, string _note_no, string _company)
    {
    ...
    {
        int _pagesize;
        if (int.TryParse(Utils.GetCookie("remove_add_page_size"), out _pagesize))
        {
            if (_pagesize > 0)
            {
                return _pagesize;
    ...
    {
        //Response.Redirect(Utils.CombUrlTxt("remove_add.aspx", "product_category_id={0}&note_no={1}", this.product_category_id.ToString(), txtNote_no.Text));
        Response.Redirect(Utils.CombUrlTxt("remove_add.aspx", "product_category_id={0}&note_no={1}&ompany={2}", this.ddlproduct_category_id.SelectedValue, txtNote_no.Text, this.ddldepot_category_id.SelectedValue));
    }
    ...
    {
        //Response.Redirect(Utils.CombUrlTxt("remove_add.aspx", "product_category_id={0}&note_no={1}", this.ddlproduct_category_id.SelectedValue, txtNote_no.Text));
        Response.Redirect(Utils.CombUrlTxt("remove_add.aspx", "product_category_id={0}&note_no={1}&Company={2}", this.ddlproduct_category_id.SelectedValue, txtNote_no.Text, this.ddldepot_category_id.SelectedValue));
    }
    ...
    protected void txtPageNum_TextChanged(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
        {
            if (_pagesize > 0)
            {
                Utils.WriteCookie("remove_add_page_size", _pagesize.ToString(), 14400);
            }
        }
        Response.Redirect(Utils.CombUrlTxt("remove_add.aspx", "product_category_id={0}&note_no={1}&Company={2}", this.product_category_id.ToString(), txtNote_no.Text, this.ddldepot_category_id.SelectedValue));
    }
