...
public partial class depotmanager_order_list_wc : System.Web.UI.Page
{
    ...
    private void RptBind(string _strWhere, string _orderby)
    {
        ...
        string pageUrl = Utils.CombUrlTxt("order_list_wc.aspx", "depot_category_id={0}&depot_id={1}&status={2}&note_no={3}&page={4}", this.depot_category_id.ToString(), this.depot_id.ToString(), this.status.ToString(), txtNote_no.Text, "__id__");
    ...
    {
        int _pagesize;
        if (int.TryParse(Utils.GetCookie("dwc_order_page_size"), out _pagesize))
        {
            if (_pagesize > 0)
            {
                return _pagesize;
    ...
    {
        Response.Redirect(Utils.CombUrlTxt("order_list_wc.aspx", "depot_category_id={0}&depot_id={1}&status={2}&note_no={3}&start_time={4}&stop_time={5}", this.depot_category_id.ToString(), this.depot_id.ToString(), this.status.ToString(), txtNote_no.Text, this.txtstart_time.Value, this.txtstop_time.Value));
    }
    ...
    protected void txtPageNum_TextChanged(object sender, EventArgs e)
    {
        ...
        if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
        {
            if (_pagesize > 0)
            {
                Utils.WriteCookie("dwc_order_page_size", _pagesize.ToString(), 14400);
            }
        }
        Response.Redirect(Utils.CombUrlTxt("order_list_wc.aspx", "depot_category_id={0}&depot_id={1}&status={2}&note_no={3}&start_time={4}&stop_time={5}", this.depot_category_id.ToString(), this.depot_id.ToString(), this.status.ToString(), txtNote_no.Text, this.txtstart_time.Value, this.txtstop_time.Value));
