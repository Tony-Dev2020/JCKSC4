...
public partial class purchase_purchase_request_edit : System.Web.UI.Page
{
    protected int page;
    ...
    protected ps_rfq modelRFQ = new ps_rfq();
    ...
    {
        int recordCount = 0;
        DataSet dsRFQ = modelRFQ.GetListByRFQVendorID(_id, out recordCount);
        this.litRFQNum.Text = dsRFQ.Tables[0].Rows[0]["RFQNum"].ToString();
        this.litRFQDate.Text = dsRFQ.Tables[0].Rows[0]["RFQDate"].ToString();
        this.litRFQcomment.Text = dsRFQ.Tables[0].Rows[0]["CommentText"].ToString();
        this.txtVendorRemark.Text = dsRFQ.Tables[0].Rows[0]["VendorRemark"].ToString();
        this.txtVendorUnitPrice.Text = dsRFQ.Tables[0].Rows[0]["VendorUnitPrice"].ToString();
    ...
    {
        //检测确认日期
        if (txtVendorRemark.Text.Trim() == "")
        {
            mym.JscriptMsg(this.Page, "请输入回复内容！", "", "Error");
            return;
        }
        ...
        mym.JscriptMsg(this.Page, "回复成功！", "purchase_request_edit.aspx?action=Edit&id=" + this.id.ToString() + "", "Success");
    }
    //小数位是0的不显示
